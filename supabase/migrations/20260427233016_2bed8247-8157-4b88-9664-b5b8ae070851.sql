...
BEGIN
  ...
  -- Verificar se é administrador (assumindo que profiles tem role ou is_admin)
  -- Se não houver coluna is_admin, podemos verificar via metadados do auth ou tabela de roles
  -- Por segurança, vamos tentar detectar se ele é admin
  BEGIN
  ...
  -- Status permitidos para lances diretos
  IF v_lot.status IN ('sold', 'passed', 'finished', 'arrematado') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Este lote já foi finalizado ou arrematado e não aceita mais lances.');
  END IF;

  -- Se for 'upcoming' (ou qualquer status que não seja 'active'/'live'), verificar pré-lance
  IF v_lot.status NOT IN ('active', 'live', 'recebendo_lances') THEN
    IF NOT COALESCE(v_lot.allows_pre_bidding, true) THEN
      RETURN jsonb_build_object('success', false, 'message', 'Este lote ainda não está aberto para lances. O pré-lance não está habilitado para este lote.');
    END IF;
    
    -- Opcionalmente poderíamos verificar se o evento já começou, mas v_lot.status deve ser 'active' se o leiloeiro abriu o lote.
  END IF;

  -- Validar valor do lance
  IF p_amount <= COALESCE(v_lot.current_price, v_lot.starting_price, 0) THEN
    RETURN jsonb_build_object('success', false, 'message', 'Lance insuficiente. O valor atual é ' || to_char(COALESCE(v_lot.current_price, v_lot.starting_price, 0), 'L999G999G990D99') || '. Tente um valor superior.');
  END IF;

  -- Verificar incremento mínimo
  IF (p_amount - COALESCE(v_lot.current_price, v_lot.starting_price, 0)) < COALESCE(v_lot.bid_increment, 0) THEN
  ...
  -- ESCALA DE RISCO (Admins ignoram)
  ...
  -- Inserir o lance
  INSERT INTO public.bids (lot_id, user_id, amount, bid_type, ip_address)
  VALUES (p_lot_id, v_user_id, p_amount, p_bid_type, p_session_id);

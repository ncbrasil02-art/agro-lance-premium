...
BEGIN
  ...
  -- SEGURANÇA MÁXIMA: Bloquear lances se o lote não estiver ATIVO
  -- Adicionado check explícito para 'sold' e 'passed' por precaução
  IF v_lot.status != 'active' OR v_lot.status = 'sold' OR v_lot.status = 'passed' OR v_lot.status = 'finished' THEN
    RETURN jsonb_build_object('success', false, 'message', 'Este lote foi finalizado ou arrematado e não aceita mais lances.');
  END IF;

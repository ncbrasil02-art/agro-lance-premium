...
import { events, lots, formatDateBR } from "@/lib/mock-data";
...
function EventDetail() {
  ...
  const eventLots = lots.filter((l) => l.eventId === event.id);
        ...
        <div className="container mx-auto -mt-32 px-4 relative">
          <div className="rounded-2xl border border-border bg-card p-8 shadow-elegant">
            ...
            <div className="mt-6 grid gap-4 border-t border-border pt-6 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { icon: Calendar, label: "Data", value: formatDateBR(event.date) },
                { icon: MapPin, label: "Local", value: `${event.city}/${event.state}` },
                { icon: Gavel, label: "Leiloeiro", value: event.auctioneer },
                { icon: Trophy, label: "Promotor", value: event.promoter },
              ].map((m) => (
      ...
      <section className="container mx-auto px-4 py-16">
        ...
        {eventLots.length > 0 ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {eventLots.map((l) => <LotCard key={l.id} lot={l} />)}
          </div>
        ) : (

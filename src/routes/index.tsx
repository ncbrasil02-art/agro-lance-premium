  ...
  loader: async () => {
    try {
      const results = await Promise.allSettled([
         supabase.from("events").select("id,slug,name,description,status,start_date,end_date,location,banner_url,viewers,auctioneer_name,promoter_company,show_countdown,lots!lots_event_id_fkey(id)").or("status.eq.live,status.eq.scheduled,status.eq.recebendo_lances,status.eq.incondicional,status.eq.em_condicional,status.eq.em_loteamento").gte("start_date", new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString()).order("start_date", { ascending: true }).limit(PAGE_LIMITS.HOME_EVENTS),
